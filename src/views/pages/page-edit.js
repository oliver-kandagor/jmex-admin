...
const PageEdit = () => {
    ...
    pageService
      ...
      .then((res) => {
        ...
        const data = {
          ...
          ...page?.buttons,
          active: !!page?.active,
          image: page?.galleries[0]?.path
            ? [createImage(page?.galleries[0]?.path)]
            : [],
        };
  ...
  const handleSubmit = (values, image) => {
    const body = {
      images: image?.length ? image?.map((img) => img?.name) : undefined,
  ...
  return (
    <Card title={t('edit.page')} extra={<LanguageList />}>
      {!loading ? (
